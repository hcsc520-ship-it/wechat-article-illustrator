---
name: wechat-article-illustrator
description: 为微信公众号文章分析结构、规划正文插图位置，并用 Type × Style × Palette 方法生成风格统一的配图。用户要求“微信公众号配图”“公众号文章插图”“为微信文章加图”或“生成公众号正文配图”时使用；不处理封面图设计或文章发布。
metadata:
  openclaw:
...
- Focus on inline illustrations for WeChat Official Account articles.
- Keep the article's argument, terminology, tone, and Chinese-language readability intact.
- Do not generate the article cover unless the user explicitly expands the request.
- Do not publish or modify content in WeChat; this skill only produces illustration assets, prompts, an outline, and optional Markdown image references.
...
Within the same task, reuse approval of a specific plan or an explicit instruction to skip confirmation while its scope remains unchanged, including continuation and retries. Do not ask again for the same decision. Ordinary saved preferences are not authorization. This rule grants no new cross-task opt-out. If cost, provider, upload scope, or deliverable scope changes materially, obtain approval for that change before proceeding.
...
- Skip confirmation only when the user explicitly says within the current task to do so, for example: "直接生成", "不用确认", "跳过确认", "按默认出图", or equivalent wording.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.baoyu-skills/wechat-article-illustrator/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/baoyu-skills/wechat-article-illustrator/EXTEND.md` | XDG |
| 3 | `$HOME/.baoyu-skills/wechat-article-illustrator/EXTEND.md` | User home |
...
**Hard gate**: this step is mandatory per the [Confirmation Policy](#confirmation-policy) — Steps 4+ cannot start until the user confirms here (or explicitly opts out with "直接生成" / equivalent wording within the current task).
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure wechat-article-illustrator preferences" / "重新配置"). The next run re-triggers first-time setup.
